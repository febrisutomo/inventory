                ...
                <div class="card-body">
                    <form class="form-customer" action="" method="POST">
                        <div class="form-group">
                            <select class="select-customer form-control mr-2" name="customer_select" required></select>
                        </div>
                    </form>
                </div>
            </div>
...
<script>
    ...
    $(document).ready(function() {
        ...
        if (localStorage.getItem('saleItems')) {
            items = JSON.parse(localStorage.getItem('saleItems'));
            console.log(items);
            calcTotal(items);
        }
        if (localStorage.getItem('customer')) {
            ...
            var newOption = new Option(customer.name, customer.id, false, false);
        ...
        $('.select-customer').on('change', function() {
            let data = $('.select-customer').select2('data');
            customer = {
                id: data[0].id,
                name: data[0].text,
            }
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    if (response.success) {
                        ...
                        window.location.href = '/saledetail/' + response.data;
                    } else {
        ...
        $('.btn-process').on('click', function() {
            ...
            if (customer.length == 0) {
                ...
                storeSales(JSON.stringify(sale));
            }
